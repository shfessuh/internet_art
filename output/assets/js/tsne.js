...
function Cell(obj) {
  ...
  this.gridCoords = {};
  this.x = obj.x;
  this.ty = obj.y;           // target y position
  this.y = this.ty + 20;      // initial y above target
  this.dropDelay = Math.random() * 2;  // random delay (in seconds or arbitrary units)
  ...
  this.w = obj.w;
  this.h = obj.h;
  this.opacity = 1.0;         // start fully opaque
  this.evaporationTime = 0;   // timer for evaporation (in seconds)
  this.interacted = false;    // flag to know if the cell has been clicked/interacted with
...
World.prototype.flyToCellIdx = function(idx) {
  ...
  // Mark the cell as interacted so it stops evaporating
  cell.interacted = true;
  // Reset evaporation timer and restore full opacity—even if it had faded completely
  cell.evaporationTime = 0;
  cell.opacity = 1.0;
...
World.prototype.render = function() {
  ...
  // Make sure you have initialized this.clock somewhere (e.g., this.clock = new THREE.Clock();)
  ...
  data.cells.forEach(cell => {
    if (cell.dropDelay > 0) {
      // Use delta time to decrement drop delay (ensuring timing is in seconds)
      cell.dropDelay -= dt;
    } else if (cell.y > cell.ty) {
      // Continue falling until cell reaches its target position
      cell.y -= 0.05; // You can adjust this speed as needed.
      if (cell.y < cell.ty) {
        cell.y = cell.ty;
      }
    } else {
      // Cell has landed; start evaporation if not interacted with
      if (!cell.interacted) {
        cell.evaporationTime += dt;  // Increment evaporation timer using real time
        
        // Begin fading after 60 seconds of idle time.
        if (cell.evaporationTime > 30) {
          // Fade over 10 seconds:
          // At 60 seconds, opacity is 1; at 70 seconds, opacity should be 0.
          cell.opacity = Math.max(0, 1 - ((cell.evaporationTime - 30) / 10));
          console.log(`Cell ${cell.idx} evaporationTime: ${cell.evaporationTime.toFixed(2)} opacity: ${cell.opacity.toFixed(2)}`);
        }
      }
    }
  });
  
  // Update geometry attributes for position and opacity.
  this.group.children.forEach(mesh => {
    var translationAttr = mesh.geometry.attributes.translation;
    var opacityAttr = mesh.geometry.attributes.opacity; // Ensure your geometry/shader supports per-cell opacity
    for (var i = 0; i < data.cells.length; i++) {
      translationAttr.array[i * 3 + 1] = data.cells[i].y;
      if (opacityAttr) {
        opacityAttr.array[i] = data.cells[i].opacity;
      }
    }
    translationAttr.needsUpdate = true;
    if (opacityAttr) {
